...
public interface ICashierReceiptPorts extends IOpenportService {
    ...
    @CjOpenport(usage = "营业出款")
    void cashierOutBusiness(ISecuritySession session,
                            @CjOpenportParameter(usage = "出款金额", name = "amount") long amount
    ) throws CircuitException;

    @CjOpenport(usage = "收益出款")
    void cashierOutIncome(ISecuritySession session,
                          @CjOpenportParameter(usage = "出款金额", name = "amount") long amount
    ) throws CircuitException;
}
